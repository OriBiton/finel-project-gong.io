...
class AutomaticDataCleaner:
    ...
    def remove_outliers_for_column(self, col, contamination=0.01):
        """
        ...
        try:
            ...
            if p_value > 0.05:
                ...
                self.data=self.data.drop(self.data.loc[outliers_zscore, col].index)
                ...
                self.data=self.data.drop(self.data.loc[outliers_iqr, col].index)

                iso_forest = IsolationForest(contamination=contamination, random_state=42)
                outliers_iforest = iso_forest.fit_predict(column_data.values.reshape(-1, 1)) == -1
                self.data=self.data.drop(self.data.loc[outliers_iforest, col].index)
    ...
    def clean_data(self, handle_missing=True, detect_outliers=True):
